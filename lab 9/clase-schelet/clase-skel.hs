...
instance Eq a => Eq (BST a) where
    (==) BSTNil BSTNil=True
    (==) (BSTNod p1 l1 r1) (BSTNod p2 l2 r2)=p1 == p2 && l1 == l2 && r1 == r2
    (==) _ _=False
...
printAR :: Show a => Int -> BST a -> [Char]
printAR _ BSTNil = ""
printAR level (BSTNod root l r) = (replicate level '\t') ++ (show root) ++ "\n" 
    ++ (printAR (level + 1) l) 
    ++ (printAR (level + 1) r) 

instance Show a => Show (BST a) where
    show BSTNil = ""
    show (BSTNod root l r) = printAR 0 (BSTNod root l r)
check2 :: TestData
check2 = tests_ 2 $
    [
        testVal "show tree" "7\n\t4\n\t\t2\n\t\t\t1\n\t\t\t3\n\t12\n\t\t10\n\t\t\t8\n\t\t15\n" $ show root
...
instance Invertible (BST a) where
    ...
    invert (BSTNod a l r) = BSTNod a (invert r) (invert l)
