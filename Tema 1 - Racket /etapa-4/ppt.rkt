...
(define (lista-multi n)
  (list (multiply T1 (car n)) (multiply T2 (car n)) (multiply T3 (car n))  )
 )


(define ppt-stream-in-tree-order
  (let loop ((n '((3 4 5))))
    (stream-cons (car n) (loop (append (cdr n) (lista-multi n) )))))
...
(define (pairs G H)
  (let loop((g G) (h H))
    ...
    (if (not (equal? (stream-first g) (stream-first h )))
    (stream-cons(cons (stream-first g) (stream-first h)) (loop (stream-rest g) h))
    (loop G (stream-rest h))
    )
    )
...
(define gh-pairs-stream
  (stream-filter (lambda (a) (if (equal? (gcd  (car a) (cdr a)) 1) #t #f )) (pairs imp1 imp3)))
...
(define (ptb a)
  (/ (- (* (cdr a) (cdr a)) (* (car a) (car a))) 2) 
  )
(define (ptc a)
  (/ (+ (* (car a) (car a)) (* (cdr a) (cdr a))) 2) 
  )


(define ppt-stream-in-pair-order
  (stream-map (lambda (a)(list (pta a) (ptb a) (ptc a))) gh-pairs-stream ))
