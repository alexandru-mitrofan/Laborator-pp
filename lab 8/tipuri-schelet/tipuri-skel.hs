...
data BST a = BSTNod {vl :: a, lt :: (BST a), rt :: (BST a)}  | BSTNil deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil elem = BSTNod elem BSTNil BSTNil
insertElem root@(BSTNod val l r) elem
  | val == elem = root
  | val < elem = BSTNod val l (insertElem r elem)
  | val > elem = BSTNod val (insertElem l elem) r

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem root@(BSTNod val l r) elem
  | val == elem = Just elem
  | val < elem = findElem r elem
  | val > elem = findElem l elem

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNod elem l r) = (inorder l) ++ [elem] ++ (inorder r)

size :: (BST a) -> Int
size  BSTNil = 0
size (BSTNod elem l r) = 1 + (size l) + (size r)

height :: (BST a) -> Int
height BSTNil = 0
height (BSTNod elem l r) = 1 + max (height l) (height r)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced (BSTNod elem l r) = (height l) == (height r)
...
-}
data Tree a = TreeNode{num :: a,copil :: [Tree a]} deriving Show
